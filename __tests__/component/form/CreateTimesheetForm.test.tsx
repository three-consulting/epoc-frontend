...
import { testEmployee, testProject, testTimesheetAllFields, testTimesheetRequiredFields } from "../../fixtures"
...
jest.mock("@/lib/utils/fetch", () => ({
    ...
    post: async (path: string, body: object): Promise<ApiUpdateResponse<Timesheet>> => pathSpy(path) && bodySpy(body),
}))
...
test("a timesheet with the required fields only can be submitted", async () => {
    render(<CreateTimesheetForm employees={[testEmployee]} project={testProject} projectId={1} />)
    ...
    expect(bodySpy.getCalls()[0].args[0]).toStrictEqual(testTimesheetRequiredFields)
})
...
test("onCancel is invoked", async () => {
    ...
    render(
        <CreateTimesheetForm employees={[testEmployee]} project={testProject} projectId={1} onCancel={onCancelSpy} />
    )
...
test("a required field cannot be missing", async () => {
    ...
    for (const field of Object.keys(testTimesheetRequiredFields).filter((key) => key !== "project")) {
        const form = render(<CreateTimesheetForm employees={[testEmployee]} project={testProject} projectId={1} />)
